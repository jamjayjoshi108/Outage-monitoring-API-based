...
def render_aws_dashboard():
    ...
    with hdr_col1:
        if st.button("⬅️ Home", use_container_width=True, key="aws_back_home"):
    ...
    with tab1:
        ...
        if filtered_tab1.empty:
            ...
            if total_days == 1:
                ...
                st.caption(f"Range of {total_days} days selected. Feeders must have outages on at least **{noto_threshold} distinct days** to be flagged.")
            ...
            with noto_col1: selected_notorious_circle = st.selectbox("Filter by Circle:", ["All Circles"] + all_circles, index=0, key="aws_noto_circ")
            with noto_col2: selected_notorious_type   = st.selectbox("Filter by Outage Type:", ["All Types", "Planned Outage", "Power Off By PC", "Unplanned Outage"], index=0, key="aws_noto_type")
            ...
            # Circle-wise Breakdown with Drilldown
            st.subheader("🔌 Comprehensive Circle-wise Breakdown")
            st.markdown("**Click on any row inside the table below** to view Feeder drill-down details.")
            ...
            # Circle-wise Outage Summary (> 4 Hours)
            ...
            def build_circle_summary(df, label):
                ...
                summary.rename(columns={\'circle_name\': \'Circle\', \'Total_Feeders_With_Outages\': \'Total Feeders with Outages\', \'Avg_Outage_Duration_Hrs\': \'Avg Outage Duration (Hrs)\'}, inplace=True)
                summary = summary.sort_values(\'Circle\').reset_index(drop=True)
                gt = {\'Circle\': \'Grand Total\', \'Total Feeders with Outages\': source[\'feeder_name\'].nunique(), \'Avg Outage Duration (Hrs)\': round(pd.to_numeric(source[\'duration_minutes\'], errors=\'coerce\').mean() / 60, 2)}
            ...
            st.caption("Each feeder row shows Zone → Circle → Feeder → Hours. Color-coded by Zone.")
    ...
    with tab3:
        ...
        if df_ptw_master.empty:
            ...
            if filtered_ptw.empty:
                st.warning("⚠️ No PTW data found for the selected time period. (PTW data available from 1st Jan 2025).")
            else:
                ...
                repeat_feeders = repeat_feeders.rename(columns={\'circle_name\': \'Circle\', \'feeders\': \'Feeder\', \'PTW_IDs\': \'Associated PTW Request Numbers\'})
                ...
                if not day_ptw.empty:
                    ...
                    final_day = day_ptw[[\'circle_name\', \'feeders\', \'start_time\', \'end_time\', \'Duration (Hours)\', \'Time Bucket\']].rename(
                        columns={\'circle_name\': \'Circle\', \'feeders\': \'Feeder\', \'start_time\': \'Start Time\', \'end_time\': \'End Time\'}
...
with open('/root/aws_dashboard.py', 'w') as f:
    f.write(aws_code)
print("aws_dashboard.py written:", len(aws_code), "chars")
