...
# ==========================================
# PAGE 2: OUTAGE MONITORING DASHBOARD
# ==========================================
        ...
        if filtered_tab1.empty:
            ...
            if not circle_piv.empty:
                ...
                if not filtered_tab1.empty:
                    ...
                    summary_df = summary_df.sort_values('Circle').reset_index(drop=True)
        
                    # Grand Total row
                    gt = {
                        'Circle': 'Grand Total',
                        'Total Feeders with Outages': filtered_tab1['feeder_name'].nunique(),
                        'Avg Outage Duration (Hrs)': round(
                            pd.to_numeric(filtered_tab1['duration_minutes'], errors='coerce').mean() / 60, 2
                        )
                    }
                    summary_df = pd.concat([summary_df, pd.DataFrame([gt])], ignore_index=True)
        
                    # Separate data rows (exclude Grand Total) for gradient scaling
                    data_rows = summary_df.index[summary_df['Circle'] != 'Grand Total']
        
                    def style_summary(df):
                        styler = df.style
        
                        # Vertical background_gradient only on data rows (not Grand Total)
                        ...
                        # Bold + highlight Grand Total row
