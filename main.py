...
# # --- CORE DATA PIPELINE (Direct API Fetch) ---
# @st.cache_data(ttl=900, show_spinner="Fetching data directly from API...")
...
#     # Calculate Last Year's dates for YoY Tab
#     try: ly_start = curr_start.replace(year=curr_start.year - 1)
#     except ValueError: ly_start = curr_start.replace(year=curr_start.year - 1, day=28)
#     try: ly_end = curr_end.replace(year=curr_end.year - 1)
#     except ValueError: ly_end = curr_end.replace(year=curr_end.year - 1, day=28)

#     # ==========================================
#     # 1. OUTAGES LOGIC
#     # ==========================================
#     curr_outages = fetch_outages_chunked(fetch_start_str, end_date_str, api_key)
#     ly_outages = fetch_outages_chunked(ly_start.strftime("%Y-%m-%d"), ly_end.strftime("%Y-%m-%d"), api_key)
    
#     df_outages = pd.DataFrame(curr_outages + ly_outages)
